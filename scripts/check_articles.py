...
def check_articles():
    ...
    with Session(engine) as session:
        ...
        if most_recent:
            logger.info(f"Most recent article: {most_recent.title} ({most_recent.published_date})")
        ...
        recent_count = session.exec(
            select(func.count()).select_from(Article).where(Article.published_date >= one_year_ago)
        ).one()
