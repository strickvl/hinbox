...
def check_articles(sample=False):
    ...
    try:
        ...
        if articles_with_dates:
            ...
            console.print(f"  Oldest: {oldest['title'][:60]}... ({oldest['published_date']})")
            console.print(f"  Newest: {newest['title'][:60]}... ({newest['published_date']})")
        ...
        if sample and rows:
            ...
            article_table.add_row("Published", sample_article.get("published_date", "N/A"))
            ...
            if metadata:
                article_table.add_row("Processed", str(metadata.get("processed", False)))
                article_table.add_row("Processing Date", metadata.get("processing_date", "N/A"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sample", action="store_true", help="Display a sample article")
