...
def import_articles():
        ...
        for i, line in enumerate(f, 1):
            ...
            try:
                ...
                if not article_data.get("content"):
                    logger.warning(
                        f"Skipping article with missing content: {article_data['url']}"
                    )
                    articles_skipped += 1
                    continue
                ...
                article = Article(
                    ...
                    content_scrape_timestamp=parse_datetime(
                        article_data.get("content_scrape_timestamp")
                    ),
                )
    ...
    logger.info(
        f"Import complete. Added {articles_added} articles, skipped {articles_skipped} articles, failed {articles_failed} articles."
    )
